...
{
    public function MonthlyAnnualProfitView()
    {
    	return view('backend.report.profit.profit_view');

    }

    public function MonthlyAnnualProfitGet(Request $request)
    {
        ...
        $html['tdsource']  .= '<td>'.'₦ '.number_format($emp_salary,2).'</td>';
        $html['tdsource']  .= '<td>'.'₦ '.number_format($total_cost,2).'</td>';
        $html['tdsource']  .= '<td>'.'₦ '.number_format($profit,2).'</td>';
        $html['tdsource']  .='<td>';
        $html['tdsource']  .='<a class="btn btn-sm btn-'.$color.'" title="PDF" target="_blanks" href="'.route("report.profit.pdf").'?start_date='.$sdate.'&end_date='.$edate.'">Print PDF</a>';
        $html['tdsource']  .= '</td>';
    ...
    public function MonthlyAnnualProfitPdf(Request $request)
    {
